...
class PSPIPainLevelClassifier(SyracuseDataset):
    """
    Class for pain level classification using PSPI features.
    Supports 3-class and 5-class classification.
    
    PSPI (Prkachin and Solomon Pain Intensity) is a facial expression-based pain score
    calculated as: AU4 + max(AU6, AU7) + max(AU9, AU10)
    """
    ...
    def prepare_features(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[str]]:
        """
        Prepare PSPI features and class labels for pain level classification.
        
        Returns:
            Tuple containing:
            - features: numpy array of shape (n_samples, 1) containing PSPI scores
        ...
        for idx, row in self.meta_df.iterrows():
            if pd.notna(row['pain_level']):
                ...
                pspi_score = self.calculate_pspi(au_df)
                pspi_scores.append(pspi_score)
                
                # Store as feature (single value)
                features_list.append([pspi_score])
        ...
        # Print PSPI statistics
        print("\nPSPI Score Statistics:")
        print(f"Mean PSPI: {np.mean(pspi_scores):.3f}")
        print(f"Std PSPI: {np.std(pspi_scores):.3f}")
        print(f"Min PSPI: {np.min(pspi_scores):.3f}")
        print(f"Max PSPI: {np.max(pspi_scores):.3f}")
        ...
        # Calculate average PSPI by pain class
        print("\n=== Average PSPI by Pain Class (3-class) ===")
        for i in range(3):
            mask = class_3 == i
            if np.any(mask):
                avg_pspi = np.mean(X[mask])
                print(f"Class {i}: {avg_pspi:.3f}")
        
        print("\n=== Average PSPI by Pain Class (5-class) ===")
        for i in range(5):
            mask = class_5 == i
            if np.any(mask):
                avg_pspi = np.mean(X[mask])
    ...
    def plot_confusion_matrices(self, results: Dict, output_dir: str = "results"):
        ...
        for problem in results.keys():
            for model_name, metrics in results[problem].items():
                ...
                # Save figure
                output_file = os.path.join(output_dir, f"{problem.replace('-', '_')}_{model_name.replace(' ', '_').replace('(', '').replace(')', '')}_cm.png")
                plt.tight_layout()
    ...
    def save_results(self, results: Dict, output_dir: str = "results", filename: str = "pspi_pain_classification_results.json"):
        """
...
def main():
    """Main function to run the PSPI pain level classification analysis."""
    # Set paths
    ...
    import argparse
    parser = argparse.ArgumentParser(description='PSPI Pain Level Classification')
    parser.add_argument('--n_splits', type=int, default=5, help='Number of folds for cross-validation')
    parser.add_argument('--random_state', type=int, default=42, help='Random seed')
    parser.add_argument('--output_dir', type=str, default='results/pspi_pain_classification', help='Output directory for results')
    args = parser.parse_args()
    
    # Initialize classifier
    classifier = PSPIPainLevelClassifier(meta_path, feature_dir, au_features_dir)
    
    # Prepare features
    X, y_3class, y_5class, video_ids = classifier.prepare_features()
    
    # Train and evaluate models
    results = classifier.train_models(X, y_3class, y_5class, n_splits=args.n_splits, random_state=args.random_state)
    
    # Plot confusion matrices
    classifier.plot_confusion_matrices(results, output_dir=args.output_dir)
    
    # Save results
    classifier.save_results(results, output_dir=args.output_dir, filename="pspi_pain_classification_results.json")
