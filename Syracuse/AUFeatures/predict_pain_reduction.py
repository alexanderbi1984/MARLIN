...
class PainReductionPredictor(SyracuseDataset):
    ...
    def prepare_features(self) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Top 5 features based on absolute effect size
        top_features = ['AU12_r', 'AU07_r', 'AU05_r', 'AU01_r', 'AU02_r']
        ...
        for pair in self.pairs:
            ...
            # Get only the top 5 real-valued AU features
            pre_df = self.au_features[pre_file]
            post_df = self.au_features[post_file]
            
            # Calculate feature differences only for top 5 features
            pre_features = pre_df[top_features].mean()
            post_features = post_df[top_features].mean()
        ...
        print(f"\nUsing top 5 features based on effect size:")
    ...
    def train_models(self, X: np.ndarray, y_binary: np.ndarray, y_abs: np.ndarray, y_percent: np.ndarray):
        ...
        models = {
            ...
            'GAM_4splines': LinearGAM(s(0) + s(1) + s(2) + s(3) + s(4), n_splines=4),
            'GAM_8splines': LinearGAM(s(0) + s(1) + s(2) + s(3) + s(4), n_splines=8),
            'GAM_4splines_lam0.1': LinearGAM(s(0) + s(1) + s(2) + s(3) + s(4), n_splines=4, lam=0.1),
            'GAM_4splines_lam10': LinearGAM(s(0) + s(1) + s(2) + s(3) + s(4), n_splines=4, lam=10)
        }
...
def main():
    ...
    predictor = PainReductionPredictor(meta_path, feature_dir, au_features_dir)
