...
class SyracuseLP(BioVidBase):
    ...
    def __init__(
        ...
        logger.info(f"SyracuseLP Dataset '{split}' initialized with {len(self.name_list)} clips.")
        if not self.metadata:
             # print(f"Warning: Metadata dictionary for {split} dataset is empty!")
             logger.warning(f"Metadata dictionary for {split} dataset is empty!")
        elif not self.name_list:
             # print(f"Warning: Name list for {split} dataset is empty!")
             logger.warning(f"Name list for {split} dataset is empty!")
    ...
    def _validate_namelist(self):
        ...
        if original_count != len(valid_names):
            ...
            logger.info(f"  SyracuseLP ({self.split}) Validation: Filtered name_list from {original_count} to {len(valid_names)}")
            if missing_meta > 0: logger.info(f"    - Skipped {missing_meta} due to missing metadata.") # Changed to info
            if missing_features > 0: logger.info(f"    - Skipped {missing_features} due to missing feature files.") # Changed to info
            if missing_pain_level > 0: logger.info(f"    - Skipped {missing_pain_level} due to missing 'pain_level' key.") # Changed to info
            if invalid_pain_level > 0: logger.info(f"    - Skipped {invalid_pain_level} due to invalid 'pain_level' format.") # Changed to info

        self.name_list = valid_names
        if not self.name_list:
             # print(f"  WARNING: SyracuseLP ({self.split}) name_list is empty after validation!")
             logger.warning(f"SyracuseLP ({self.split}) name_list is empty after validation!")
    ...
    def _normalize_features(self, features: np.ndarray) -> np.ndarray:
        ...
        if features is None or features.size == 0:
             ...
             logger.warning(f"Input features are None or empty for a sample in {self.split}. Returning zeros.") # Added context
        ...
        if features.ndim == 1:
             if features.shape[0] == target_frames * feature_dim:
                 ...
                 logger.warning(f"Received 1D features of shape {features.shape} in {self.split}. Repeating to {target_frames} frames.")
                 ...
                 logger.warning(f"Received unexpected 1D features shape {features.shape} in {self.split}. Returning zeros.")
             ...
             logger.warning(f"Input features have unexpected ndim {features.ndim} in {self.split}. Returning zeros.")
             return np.zeros((target_frames, feature_dim), dtype=features.dtype)

        if features.shape[1] != feature_dim:
            # print(f"Warning: Feature dimension is {features.shape[1]}, expected {feature_dim}. Returning zeros.")
            logger.warning(f"Feature dimension is {features.shape[1]}, expected {feature_dim} in {self.split}. Returning zeros.")
            return np.zeros((target_frames, feature_dim), dtype=features.dtype)
        ...
        if n_frames == target_frames:
            ...
            return features[indices]
             ...
             if n_frames == 0:
                  ...
                  logger.warning(f"Input features are empty after checks for a sample in {self.split}. Returning zeros.")
                  return np.zeros((target_frames, feature_dim), dtype=features.dtype)
    ...
    def __getitem__(self, index: int):
        ...
        try:
            ...
            logger.error(f"Feature file not found: {feature_path}") # Use error
            ...
            logger.error(f"Error loading feature file {feature_path}: {e}") # Use error
            dummy_features = torch.zeros((4 if self.temporal_reduction == "none" else 1, 768), dtype=torch.float32)
            dummy_label = torch.tensor(-1, dtype=torch.long)
            return dummy_features, dummy_label
        ...
        try:
            ...
            logger.error(f"Metadata Key Error for {filename}: {e}. Check metadata structure.")
            dummy_features = torch.zeros((4 if self.temporal_reduction == "none" else 1, 768), dtype=torch.float32)
            dummy_label = torch.tensor(-1, dtype=torch.long)
            return dummy_features, dummy_label
        except Exception as e:
            # print(f"Error processing metadata or label for {filename}: {e}")
            logger.error(f"Error processing metadata or label for {filename}: {e}")
            dummy_features = torch.zeros((4 if self.temporal_reduction == "none" else 1, 768), dtype=torch.float32)
            dummy_label = torch.tensor(-1, dtype=torch.long)
            return dummy_features, dummy_label
...
class SyracuseDataModule(LightningDataModule):
    ...
    def __init__(self, root_dir: str,          # Root directory of data
        ...
        try:
            ...
            logger.info(f"Initializing MarlinFeatures with base directory: {self.marlin_base_dir}")
            if not os.path.exists(self.marlin_base_dir):
                 # print(f"Error: Provided marlin_base_dir does not exist: {self.marlin_base_dir}")
                 logger.error(f"Provided marlin_base_dir does not exist: {self.marlin_base_dir}")
            else:
                 ...
                 if not os.path.exists(metadata_json_path):
                      ...
                      logger.error(f"clips_json.json not found in marlin_base_dir: {self.marlin_base_dir}")
                 else:
                      self.marlin_features = MarlinFeatures(self.marlin_base_dir)
                      # print("MarlinFeatures initialized successfully.")
                      logger.info("MarlinFeatures initialized successfully.")
            ...
            logger.error(f"Error initializing MarlinFeatures (FileNotFound): {e}")
            # print("Please ensure the marlin_base_dir is correct and contains clips_json.json.")
            logger.error("Please ensure the marlin_base_dir is correct and contains clips_json.json.")
        except Exception as e:
             # print(f"An unexpected error occurred during MarlinFeatures initialization: {e}")
             logger.exception(f"An unexpected error occurred during MarlinFeatures initialization: {e}") # Use exception to log traceback
    ...
    def _load_and_prepare_metadata(self) -> bool:
        ...
        if self.marlin_features and hasattr(self.marlin_features, 'clips_metadata') and self.marlin_features.clips_metadata:
            ...
            logger.info(f"Loaded metadata using initialized MarlinFeatures instance ({len(self.all_metadata)} entries).")
            return True
        else:
            # print("Error: MarlinFeatures instance not initialized or has no metadata.")
            logger.error("MarlinFeatures instance not initialized or has no metadata.")
            return False
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        logger.info(f"Processing {len(self.all_metadata)} clips from metadata for SyracuseDataModule setup...")
        ...
        for filename, clip_data in self.all_metadata.items():
            ...
            if video_type == 'original':
                ...
                if pain_level_value is not None:
                    try:
                        ...
                        if items_with_valid_pain % 100 == 1: # Print occasionally
                            ...
                            logger.debug(f"  [DEBUG setup loop1] Added pain: video_id={video_id} (type {type(video_id)}), pain={pain_level}") # Use logger.debug
                    except (ValueError, TypeError):
                ...
                logger.warning(f"Unknown video_type '{video_type}' for clip {filename}. Skipping.")
        ...
        logger.info(f"Finished processing metadata:")
        # print(f"  - Original clips found: {len(self.original_clips)}")
        logger.info(f"  - Original clips found: {len(self.original_clips)}")
        # print(f"  - Augmented clips found: {len(self.augmented_clips)}")
        logger.info(f"  - Augmented clips found: {len(self.augmented_clips)}")
        # print(f"  - Unique video IDs found: {len(self.video_ids)}")
        logger.info(f"  - Unique video IDs found: {len(self.video_ids)}")
        if clips_missing_info > 0: logger.info(f"  - Clips skipped (missing info): {clips_missing_info}")
        if clips_missing_label > 0: logger.info(f"  - Clips skipped (missing/invalid pain label): {clips_missing_label}") # Adjusted message
        ...
        logger.info("SyracuseDataModule setup complete.")
        # <<< Add Debug Print 2 >>>
        ...
        logger.debug(f"  [DEBUG setup before loop2] video_pain_levels keys ({len(video_pain_levels)}): {list(video_pain_levels.keys())[:20]}...") # Use logger.debug
        logger.debug(f"  [DEBUG setup before loop2] video_ids ({len(self.video_ids)}): {list(self.video_ids)[:20]}...") # Use logger.debug
        ...
        for vid in sorted(list(self.video_ids)):
            ...
            if processed_vids_in_loop2 % 10 == 1: # Print occasionally
                 ...
                 logger.debug(f"  [DEBUG setup loop2] Checking vid={vid} (type {type(vid)}). Key exists: {key_exists}, List not empty: {list_not_empty}") # Use logger.debug
        ...
        if videos_without_valid_pain_for_avg > 0:
            ...
            logger.warning(f"{videos_without_valid_pain_for_avg} video IDs had no original clips with valid 'pain_level' for averaging. They won't be used for stratified splitting.")
        ...
        logger.info(f"Derived {len(self.video_id_labels)} video_id -> class label mappings for stratification.")
